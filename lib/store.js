...
async function updateApplicationStatus(id, updates) {
  await ensureSchema();
  const rows = await sql`SELECT data FROM applications WHERE id = ${id}`;
  if (!rows[0]) return null;
  const merged = { ...rows[0].data, ...updates };
  await sql`
    UPDATE applications SET data = ${JSON.stringify(merged)}::jsonb WHERE id = ${id}
  `;
  return merged;
}
...
async function getStatusCounts() {
  await ensureSchema();
  const rows = await sql`
    SELECT COALESCE(data->>'status', 'yuborilgan') AS status, COUNT(*)::int AS count
    FROM applications
    GROUP BY status
  `;
  const counts = {};
