...
const { kvSessionStore, nextApplicationId, saveApplication } = require('./store');
...
async function stepPropertyType(ctx) {
  ...
  await ctx.reply("5️⃣ Narxini yozing (so'mda). Masalan: 300000000");
  return ctx.wizard.next();
}
...
async function stepInteriorPhotos(ctx) {
  if (ctx.updateType === 'callback_query') {
    ...
    if (ctx.callbackQuery.data === 'int_done') {
      ...
      await ctx.reply(
        "9️⃣ Hujjatlarni yuklang (mulkchilik guvohnomasi va h.k.) — bu ixtiyoriy.\n\n" +
...
function createBot() {
  ...
  bot.use(session({ store: kvSessionStore }));
